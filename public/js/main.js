...
jQuery(document).ready(function(){
    ...
    jQuery('.masonry').each(function(){
		...
		if($container.find('img').length == 0) $element = jQuery('<img />');
        
		imagesLoaded($element,function(instance){
			$container.isotope({
			   itemSelector: '.post',
			   masonry: { columnWidth: jQuery('.posts').width() / $cols }
			});
...
jQuery(document).ready(function () {
    ...
    jQuery('.nav-menu').each(function(){
		...
		filter: ':not(.external)',
			...
			$('#nav-menu a').css('text-decoration', 'none');
		}
        });
    });
	...
	jQuery('.video-section').each(function() {
		...
		if(screen.width < 800 || iedetect(8) || iedetect(7) || 'ontouchstart' in window) {
			var videoPoster = jQuery(this).attr('data-poster');
			$(this).css({
	...
	//---- Services mouse over color change on header
	jQuery('#services .owl-item').mouseover(function(){
		$(this).find("h4").addClass("accent-color");
	}).mouseout(function(){
		$(this).find("h4").removeClass("accent-color");
	});
...
window.onload = function(e) {
  ...
  var colors = ["blue", "orange", "yellow", "torquise"];  // Set random color values
  var title = colors[Math.round(Math.random() * (colors.length -1))];
  setActiveStyleSheet(title);  // Or set theme color by editing the title variable
}
...
jQuery(document).ready(function(){
	...
	*/
	...
	$('#logo.visible-lg a[href*=#]:not([href=#]), #logo.visible-sm a[href*=#]:not([href=#]), #goTop, .goAbout').click(function() {
		if (location.pathname.replace(/^\//,'') == this.pathname.replace(/^\//,'') && location.hostname == this.hostname) {
		  var target = $(this.hash);
		  target = target.length ? target : $('[class=' + this.hash.slice(1) +']');
		  if (target.length) {
